    ...
    <div>
      ...
      <div style={styles.content}>
        <div style={styles.infoTextContainer}>
          ...
          <p>
            This page requires authentication. It will do a <strong>custom/dynamic</strong>
            server-side redirect (307) to the login page if the user is not authenticated.
          </p>
          <p>
            The custom logic also does the following (which does not actually work, it's only for the purpose of illustrating what can be done):
            <ul>
              <li>If authenticated, but no email, it will generate a 404 page.</li>
              <li>If authenticated, but email is not verified, it performs a custom redirect to the login page and injects query parameters in the URL.</li>
            </ul>
          </p>
          <p>This page leverages the standard `redirect` and 'notFound' objects returned by `getServerSideProps` to perform the custom routing.</p>
...
export const getServerSideProps = withAuthUserSSR()(async (ctx) => {
  ...
  if (!AuthUser.emailVerified) {
    return {
      redirect: {
        destination: `/login?verifyEmail=true&thenGoToPage=${encodeURIComponent(ctx.resolvedUrl)}`,
        permanent: false,
      },
    };
  }
